...
const Analytics = () => {
  const { user } = useAuth()
  const { members, invoices, checkIns, activities, followUps } = useDataStore()
  ...
  const exportData = () => {
    ...
    a.href = url
    a.download = `tristar-analytics-${format(new Date(), 'yyyy-MM-dd')}.json`
          ...
          <CardContent>
            <div className="space-y-4">
              <div className="flex items-center justify-between p-3 bg-green-50 dark:bg-green-900/20 rounded-lg">
                <div>
                  <p className="font-medium text-gray-900 dark:text-gray-100">Paid Revenue</p>
                  <p className="text-sm text-gray-600 dark:text-gray-400">Completed payments</p>
                </div>
                <div className="text-right">
                  <p className="text-xl font-bold text-green-600 dark:text-green-400">₹{totalRevenue.toLocaleString()}</p>
                  <p className="text-xs text-gray-600 dark:text-gray-400">
